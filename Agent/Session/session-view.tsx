...
export const SessionView = ({ appConfig, onAnimationComplete }: SessionViewProps) => {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-gradient-to-br from-slate-900 via-teal-900 to-slate-900"> {/* Teal change */}
      {/* Background effects */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(45,212,191,0.2),transparent_50%)]" /> {/* Teal change */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_80%,rgba(251,191,36,0.15),transparent_50%)]" /> {/* Amber/Gold change */}
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <div className="mb-2 flex items-center gap-3">
                  <span className="inline-flex items-center rounded-full bg-teal-500/20 px-3 py-1 text-xs font-semibold text-teal-300"> {/* Teal change */}
                    LIVE
                  </span>
              ...
              <div className="flex items-center gap-2">
                {isHostSpeaking && (
                  <motion.div
                    ...
                    className="flex items-center gap-2 rounded-full bg-teal-500/20 px-4 py-2" // Teal change
                  >
                    <div className="h-2 w-2 rounded-full bg-teal-400" /> {/* Teal change */}
                    <span className="text-xs font-medium text-teal-300">Host speaking</span> {/* Teal change */}
                  </motion.div>
                )}
              </div>
            </div>
                    ...
                    >
                      <div
                        className={cn(
                          ...
                          isHost
                            ? 'bg-teal-500/20 text-white' // Teal change for Host
                            : isLocal
                            ? 'bg-emerald-500/20 text-white' // Emerald change for Local Player
              ...
              <div className="flex items-center gap-3">
                ...
                <motion.button
                  ...
                  className={cn(
                    ...
                    isMuted
                      ? 'bg-red-500/20 text-red-400 hover:bg-red-500/30' // Red for muted (kept as standard warning color)
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={handleLeave}
                  className="flex h-12 items-center gap-2 rounded-full bg-red-500/20 px-6 text-red-400 transition-all hover:bg-red-500/30" // Red for leave (kept as standard warning color)
                >
